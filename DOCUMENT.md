...
8. [退休規劃試算](#八退休規劃試算-retirement)
9. [投資績效分析](#九投資績效分析-investment-analysis)
10. [資料工具](#十資料工具-data-tools)
11. [通用功能](#十一通用功能)
12. [資料層 API 整理](#十二資料層-api-整理)
13. [IndexedDB Schema 版本歷程](#十三indexeddb-schema版本歷程)
14. [JSON 匯出入格式](#十四json-匯出入格式)
15. [計算公式與邏輯說明](#十五計算公式與邏輯說明)
16. [技術注意事項](#十六技術注意事項)
...
└── features/
    ...
    ├── investmentAnalysis/
    │   ├── InvestmentAnalysisPage.tsx  # 投資績效分析主頁（頁籤 + 篩選 + 更新股價）
    │   ├── types/
    │   │   └── investmentAnalysis.ts   # AssetAnalysisRow、FundAnalysisRow 等型別
    │   └── components/
    │       ├── AssetAnalysisTable.tsx          # 一般標的分析表
    │       ├── FundAnalysisTable.tsx           # 基金分析表
    │       ├── InvestmentAnalysisSummaryCards.tsx  # 摘要 KPI 卡片
    │       ├── InvestmentWarningsPanel.tsx     # 風險警示分頁
    │       └── AnalysisFilters.tsx             # 篩選面板
    ├── rebalance/
    │   └── RebalancePage.tsx        # 資產再平衡表（圓餅圖 + 目標設定 + 排序）
    ├── cashflow/
    │   └── CashflowPage.tsx         # 收入與日常消費表（全部功能整合）
    ├── retirement/
    │   ├── RetirementPage.tsx       # 退休規劃試算（蒙地卡羅模擬 + 帶入按鈕）
    │   └── monteCarloEngine.ts      # 蒙地卡羅純計算引擎
    └── dataTools/
        └── DataToolsPage.tsx        # 資料工具（備份強化 + 健康檢查）
```
...
- 現有資金支持輸入負數（機動帳戶功能需要）
- 輸入負數時欄位下方顯示橙色提示；按「儲存」後彈出確認對話框，確認存入負數餘額
...
| 目標設定 CRUD | 新增 / 編輯 / 刪除各類型目標比例與容忍誤差；所有欄位可排序；**預設依目標比例降序排列** |
...
| 卡片 | 說明 | 顏色規則 |
|------|------|---------|
| 現金帳戶 (TWD) | 所有 `cash` 類型帳戶餘額換算 TWD 加總（使用即時匯率）；**點擊可展開各帳戶餘額構成明細** | 固定藍色 |
...
| 金額 | 數字輸入，不可為負數 |
| 幣別 | TWD / USD / JPY / CNY |
| 匯率（對 TWD） | 僅非 TWD 幣別時顯示；切換幣別時自動帶入 DB 廽存匃婦方式（USD→usdRate）；幫廷小按鈕可手動覆寫 |
...
|------|------|
| 目前年齡 / 退休年齡 / 預期壽命 | 整數，歲 |
| 目前總資產 | 退休前起始資產（TWD）；可點擊**「帶入總資產」**自動填入再平衡表現在總市值 |
| 每月投入 | 退休前每月定期投入金額；可點擊**「帶入月結餘」**自動填入收支表最新月份結餘 |
| 預期年化報酬率 | 退休前/後年化報酬率（%） |
| 報酬率年化波動率 | 標準差（%） |
| 退休後每月支出 | 今日幣值月支出（TWD） |
| 通膨率 | 年通膨率（%），用於遞增退休後月支出 |
...
| 指標 | 說明 |
|------|------|
| 退休成功率 | 資產未在壽命結束前耗盡的模擬比例（%） |
| 退休時資產中位數 | 退休年齡時資產的 P50 值（TWD） |
| 資產耗盡年齡中位數 | 失敗情境下資產耗盡時的年齡中位數 |

### 8.3 圖表

Recharts ComposedChart（Area + Line），橫軸為年齡，縱軸為資產 TWD，顯示：

| 系列 | 說明 |
|------|------|
| P10–P90 面積帶 | 10/25/50/75/90 百分位資產走勢 |
| P50 中位數線 | 粗藍線 |
| 退休年齡垂直參考線 | 橘色虛線 |

### 8.4 計算引擎（`monteCarloEngine.ts`）

- Box-Muller 常態分佈亂數產生器
- 逐月模擬（退休前：複利成長 + 每月投入；退休後：複利成長 − 通膨遞增月支出）
- 輸入 `SimParams`，輸出 `SimResult`（含 `chartData: ChartPoint[]`）

---

## 九、投資績效分析（`/investment-analysis`）

以投資資產管理資料為基礎，自動計算每筆資產的報酬率、配息資訊，並整合收支紀錄中的「基金利息」與「股利」紀錄（透過 `linkedTicker` 關聯），提供多維度投資診斷。

### 9.1 頁籤

| 頁籤 | 說明 |
|------|------|
| 一般標的 | 台股、美股、日股、ETF 等非基金資產分析表 |
| 基金 | 基金資產分析表（含配息率、殖利率） |
| 風險警示 | 自動偵測淨值跌幅、集中度等問題 |

### 9.2 摘要 KPI 卡片

| 指標 | 說明 |
|------|------|
| 總市值 (TWD) | 全部非現金資產市值換算 TWD |
| 總成本 (TWD) | 全部資產成本基礎 TWD 加總 |
| 未實現損益 (TWD) | 市值 − 成本（台股慣例：正紅負綠） |
| 累積配息 (TWD) | 全部基金「基金利息」收支紀錄加總，轉換 TWD |
...
| 市值 (TWD) | qty × 現價 × 目前匯率 |
| 未實現損益 / % | 市值 − 成本 |
| 累積股息 | 同 `linkedTicker` 的「股利」收支紀錄加總（原幣） |
| 含息總報酬 / % | 未實現損益 + 累積股息（轉 TWD）|
| 占比 | 該資產市值 / 全部投資市值 |
...
| 欄位 | 公式 | 幣別 |
|------|------|------|
| 累積淨配息 | `distributionsByTicker[ticker]` 加總 | 原幣（OC） |
| 配息率% | 累積淨配息OC / 成本OC | — |
| 含息總報酬 / % | 淨值損益TWD + 累積淨配息×currentFx | TWD |
| 近12月配息 | `trailing12ByTicker[ticker]` 加總（近 365 天） | 原幣（OC） |
| 近12月殖利率% | 近12月配息OC / 市值OC | — |
| 成本殖利率% | 近12月配息OC / 成本OC | — |

> **匯率統一原則**：配息率、殖利率的分子分母均使用原幣（OC）計算，避免不同時間點匯率造成失真。  
> `totalNetDistributionsTWD = OC × currentFx`（僅用於 KPI 卡片加總顯示）。

### 9.5 linkedTicker 資料關聯

- 收支紀錄（`IncomeExpenseRecord`）與固定月計畫（`MonthlyExpensePlan`）均有 `linkedTicker?: string` 欄位
- 新增/編輯收支時，若分類為「基金利息」或「股利」，顯示代號下拉選單（來自資產管理的 ticker 清單）
- 匯入固定月計畫時，若計畫有 `linkedTicker`，匯入的收支紀錄亦帶入該代號
- 分析頁以 `linkedTicker` 為鍵值 group by，累加配息金額

### 9.6 風險警示規則

| 警示代碼 | 條件 | 等級 |
|---------|------|------|
| `MISSING_CURRENT_PRICE` | 現價 ≤ 0 | warning |
| `NAV_DRAWDOWN_25` | 基金淨值跌幅 > 25% | danger |
| `NAV_DRAWDOWN_15` | 基金淨值跌幅 > 15% | warning |
| `SINGLE_FUND_WEIGHT_HIGH` | 單一基金佔比 ≥ 25% | warning |
| `SINGLE_ASSET_WEIGHT_HIGH` | 單一標的佔比 ≥ 30% | warning |
| `NEGATIVE_TOTAL_RETURN` | 總報酬率 < 0 | warning |

### 9.7 一鍵更新現價

與投資紀錄表共用同一套更新邏輯（TWSE / Yahoo Finance / allorigins / Stooq），批次更新目前持有（quantity > 0）的基金與一般標的，顯示成功/失敗/略過結果列表。
...
| 功能 | 說明 |
|------|------|
| 距上次備份天數 | 從 `localStorage` 讀取上次備份時間；≥ 30 天（或從未備份）顯示橘色警示 |
| 備份標籤 | 選填；預設使用日期時間戳 |
| 立即備份 | 匯出全部 IndexedDB 資料為 JSON 並下載；同時記錄備份歷史 |
| 備份歷史 | 最多保留 20 筆；顯示標籤、時間、資料量 |

備份紀錄存於 `localStorage`（key：`fa_backup_history`、`fa_last_backup_at`）。
...
| 問題類型 | 說明 |
|----------|------|
| 孤立投資交易（無對應資產） | 交易的 `assetId` 找不到對應資產 |
| 孤立投資交易（無對應帳戶） | 交易的 `accountId` 找不到對應帳戶 |
| 孤立收支紀錄（無對應分類） | 收支的 `categoryId` 找不到對應分類 |
| 孤立固定月計畫（無對應分類） | 計畫的 `categoryId` 找不到對應分類 |
| 帳戶轉帳引用不存在帳戶 | 轉帳的 `fromAccountId` 或 `toAccountId` 找不到帳戶 |
| 帳戶餘額為負數 | 帳戶 balance < 0（警告，非錯誤） |
| 資產數量為負數 | 資產 quantity < 0 |
| 資產 Ticker 空白 | 非現金資產未填入代碼 |
| 重複 Ticker（同市場） | 同一市場內有相同代碼的不同資產 |
...
| StatCard 点擊事件 | `StatCard` 支援 `onClick` prop，傳入後顯示 hover 藍色外框可點擊（現金帳戶卡片使用） |

---

## 十二、資料層 API 整理

### 12.1 Repository（CRUD 存取層）
...
### 12.4 InvestmentAnalysis Service（`src/data/services/investmentAnalysis.ts`）

| 函式 | 輸入 | 輸出 | 說明 |
|------|------|------|------|
| `getTotalInvestmentMarketValueTWD(assets, exchangeRate)` | 資產陣列、匯率 | `number` | 所有非現金資產市值換算 TWD 加總 |
| `calculateAssetAnalysisRows(assets, exchangeRate, totalMV, dividendsByTicker)` | 資產陣列、匯率、總市值、股息Map | `AssetAnalysisRow[]` | 一般標的分析列 |
| `calculateFundAnalysisRows(assets, exchangeRate, totalMV, distributionsByTicker, trailing12ByTicker)` | 資產陣列、匯率、總市值、配息Map×2 | `FundAnalysisRow[]` | 基金分析列（含3種殖利率）|
| `calculateInvestmentAnalysisSummary(assetRows, fundRows)` | 兩個分析列陣列 | `InvestmentAnalysisSummary` | 摘要 KPI 加總 |
| `generateInvestmentWarnings(row, context)` | 分析列、環境 | `InvestmentWarning[]` | 產生風險警示 |

### 12.5 Sorting 工具（`src/lib/sorting.ts`）
  ...
  "incomeExpenseRecords": [
    {
      ...
      "currency": "TWD",
      "fxRateToBase": 1.0,
      "linkedTicker": "AAPL",   // 選填；分類為「基金利息」或「股利」時填入，供投資績效分析使用
      "note": "備註",
      "createdAt": "ISO8601",
      "updatedAt": "ISO8601"
    }
  ...
  "monthlyExpensePlans": [
    {
      ...
      "plannedAmount": 10000,
      "currency": "TWD",
      "linkedTicker": "AAPL",   // 選填；與收支紀錄 linkedTicker 同義，匯入計畫時帶入
      "note": "備註",
      "createdAt": "ISO8601",
      "updatedAt": "ISO8601"
...
#### 成本基礎（TWD）
```
calcCostBasisTWD(asset, fallbackFx):
  有批次 → Σ lot.qty × lot.buyPrice × (lot.fxRateToBase > 0 ? lot.fxRateToBase : fallbackFx)
  無批次 → qty × buyPrice × (asset.fxRateToBase > 0 ? asset.fxRateToBase : fallbackFx)
```

#### 成本基礎（原幣，用於殖利率分母）
```
calcCostBasisOC(asset):
  有批次 → Σ lot.qty × lot.buyPrice
  無批次 → qty × buyPrice
```

#### 配息 Map 建立（原幣，不乘 fxRate）
```
distributionsByTicker[ticker] = Σ r.amount  （分類=「基金利息」且有 linkedTicker 的收支紀錄）
trailing12ByTicker[ticker]    = Σ r.amount  （同上，且 r.date >= 今天 - 12 個月）
dividendsByTicker[ticker]     = Σ r.amount  （分類=「股利」且有 linkedTicker 的收支紀錄）
```

#### 殖利率公式（全部原幣計算）
```
costBasisOC  = calcCostBasisOC(asset)
marketValueOC = qty × currentPrice

distributionReturnRate（配息率%）  = 累積淨配息OC / costBasisOC
trailing12MonthYield（近12月殖利率%）= 近12月配息OC / marketValueOC
yieldOnCost（成本殖利率%）           = 近12月配息OC / costBasisOC
```

#### 含息總報酬（以 TWD 統一計算）
```
totalNetDistributionsTWD = totalNetDistributionsOC × currentFx
totalReturnWithDistributionTWD = navPnLTWD + totalNetDistributionsTWD
totalReturnWithDistributionRate = totalReturnWithDistributionTWD / costBasisTWD
```
...
*本文件由 GitHub Copilot 依程式碼自動生成，最後更新：2026-04-30*
